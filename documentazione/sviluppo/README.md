...
Le sottocartelle `cantiere_semantico` e `validazione_rc` compaiono sia in `strumenti/` sia in `test/` intenzionalmente:
...
strumenti/validazione_rc/       = infrastruttura di validazione della RC
test/validazione_rc/            = test dell'infrastruttura e dei contratti RC
```

Non sono quindi duplicati: separare codice di collaudo e test del codice di collaudo evita che le due responsabilità vengano confuse.
