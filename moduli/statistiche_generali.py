...
GIUDIZI = (
    ("ottimali", "OTTIMA", "ottimale", "ottimali", "affinità 3", "positivo"),
    ("buone", "BUONA", "buona", "buone", "affinità 2", "positivo"),
    ("discrete", "DISCRETA", "discreta", "discrete", "affinità 1", "positivo"),
    ("accettabili", "ACCETTABILE", "accettabile", "accettabili", "neutrali", "positivo"),
    (
        "problematiche", "PROBLEMATICA", "delicata", "delicate",
        "incompatibilità 1", "problematica",
    ),
    (
        "critiche", "CRITICA", "problematica", "problematiche",
        "incompatibilità 2", "critica",
    ),
)
...
def _righe_giudizi(
    prefisso_singolare: str,
    prefisso_plurale: str,
    conteggio: dict,
) -> list[dict]:
    righe = []
    for (
        ...
        righe.append(_riga(
            chiave,
            (
                f"{prefisso_singolare} {aggettivo_singolare} ({spiegazione})"
                if valore == 1
                else f"{prefisso_plurale} {aggettivo_plurale} ({spiegazione})"
            ),
            valore,
            grassetto=grassetto,
            icona=icona,
...
def costruisci_statistiche_generali_coppie(
    ...
    totale_coppie = sum(conteggio.values())
    righe = [_riga(
        "totali",
        forma_numerata(totale_coppie, "Coppia totale", "Coppie totali"),
        totale_coppie,
    )]
    righe.extend(_righe_giudizi("Coppia", "Coppie", conteggio))
...
def costruisci_statistiche_generali_terzetti(
    ...
    totale_vicinanze = sum(conteggio.values())
    righe = [_riga(
        "totali",
        forma_numerata(
            totale_vicinanze,
    ...
    righe.extend(_righe_giudizi("Vicinanza", "Vicinanze", conteggio))
    righe.append(_riga(
        "riutilizzate",
        forma_numerata(
            riutilizzi,
            "Vicinanza riutilizzata",
    ...
    if dati["vicini_fisso_riutilizzati"]:
        ...
        etichetta_vicini = forma_numerata(
            len(dati["vicini_fisso_riutilizzati"]),
            "vicino del FISSO",
            "vicini del FISSO",
        )
        righe.append(_dettaglio(
            "dettaglio_vicino_fisso",
            f"Di cui {etichetta_vicini}: {nomi}",
            grassetto=True,
            colore="ocra",
        ))
