...
from moduli.vincoli import MotoreVincoli, MotoreVincoliConfigurato, DEBUG_MOTORE
...
class AssegnatorePosti:
    ...
    def __init__(self):
        ...
        # Conteggi ricalcolati al termine di ogni disposizione.
        self.stats = {
        ...
        if self.gestisce_trio:
            ...
            if trio_in_prima_fila:
                ...
                if studenti_no_trio:
                    ...
                    print(f"   🔒 Trio NON in prima fila: escludo dal trio gli allievi PRIMA ({nomi})")
    ...
    def _costruisci_report_diagnostico(self) -> dict:
        ...
        if info_capienza["mancanti"] > 0:
            cause_certe.append(
                f"L'aula offre {info_capienza['posti']} posti, "
                f"ma gli studenti sono {info_capienza['studenti']}: "
                f"mancano {info_capienza['mancanti']} posti."
            )

        if info_prima["impossibile_per_capienza"]:
            cause_certe.append(
                f"La prima fila offre "
                f"{info_prima['posti_utilizzabili']} posti utilizzabili, "
                f"ma {info_prima['richieste']} studenti hanno posizione "
                f"PRIMA: ci sono {info_prima['eccesso']} richieste in eccesso."
            )
        ...
        if (
            ...
            suggerimenti.append(
                "La capienza può essere sufficiente, ma la combinazione "
                "delle incompatibilità di livello 3 può impedire una "
                "suddivisione completa. Nell'Editor studenti verifica "
                "quali vincoli devono DAVVERO restare ASSOLUTI."
            )
    ...
    def _analizza_blacklist_dettagliato(self):
        ...
        print(f"   • Coppie in blacklist (classe corrente): {len(coppie_classe)}")
    ...
    def _assegna_trio_atomico_corretto(self, trio_studenti, banchi_per_fila, modalita_trio):
        ...
        print(f"   ❌ La fila target non può ospitare il trio "
    ...
    def _calcola_punteggio_adiacente_fisso(self, studente: 'Student', fisso: 'Student') -> int | float:
        ...
        Legge i vincoli del candidato verso il FISSO, perché quest'ultimo non possiede
        vincoli propri, e applica la penalità per gli utilizzi precedenti nel ruolo.
    ...
    def _calcola_statistiche_finali(self):
        """Ricalcola e stampa le statistiche della disposizione completata."""
        print("📊 STATISTICHE ASSEGNAZIONE")
        print("-" * 30)

        # Tutti i bucket vengono azzerati: il metodo è idempotente.
        self.stats['coppie_ottimali'] = 0
        self.stats['coppie_buone'] = 0
        self.stats['coppie_accettabili'] = 0
        self.stats['coppie_problematiche'] = 0
        self.stats['coppie_critiche'] = 0
