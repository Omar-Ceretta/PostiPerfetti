...
class ConfigurazioneAula:
    ...
    def crea_layout_terzetti(self, num_studenti, terzetti_per_fila=3,
        ...
        # blocchi_finali = lista dei blocchi del RESTO, ciascuno (larghezza, tipo).
        # Vuota se non c'è resto; 1 elemento per coppia/quartetto singolo; DUE
        # quartetti se il docente sceglie 'due_quartetti' (resto 2, N>=8, opt-in).
        if resto == 1:
            # Un terzetto "diventa" quartetto assorbendo l'allievo in eccesso.
            k_terzetti = num_rimanenti // 3 - 1
            blocchi_finali = [(4, 'quartetto')]
        elif resto == 2:
            if preferenza_resto2 == 'due_quartetti' and num_rimanenti >= 8:
                # I due studenti in eccesso vengono assorbiti da due quartetti.
                k_terzetti = (num_rimanenti - 8) // 3
                blocchi_finali = [(4, 'quartetto'), (4, 'quartetto')]
            else:
                # I due studenti in eccesso formano una coppia finale.
                k_terzetti = num_rimanenti // 3
                blocchi_finali = [(2, 'coppia')]
        else:  # resto == 0 -> tutti terzetti, nessun blocco finale.
            k_terzetti = num_rimanenti // 3
            blocchi_finali = []
    ...
    def _crea_fila_con_trio(self, riga_griglia, numero_fila,
        ...
        # normali (num_blocchi = max(3, posti//2)), stesso allineamento del
    ...
    def piazza_gruppi_terzetti(self, gruppi) -> dict:
        ...
        for gruppo, blocco in zip(gruppi_resto, blocchi_resto):
