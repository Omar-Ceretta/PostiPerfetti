...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QLayout, QSizePolicy
)
from PySide6.QtGui import QIcon
from PySide6.QtCore import Qt, QObject, QEvent, QSize, QTimer

from moduli.metrica_pulizia import (estrai_gruppi, adiacenze_in_fila,
                                    TIPO_TERZETTO, TIPO_QUARTETTO)
...
def conta_riutilizzate_con_foto(assegnatore, foto,
                                vicini_fisso_precedenti=None) -> dict:
    ...
    rappresenta ancora l'ordine originario. Restituisce lo stesso dizionario di
    ``conta_riutilizzate`` e riceve a parte i precedenti vicini del FISSO.
    """
    if vicini_fisso_precedenti is None:
        vicini_fisso_precedenti = set()

    def _in_foto(sa, sb):
        return tuple(sorted([sa.get_nome_completo(),
                             sb.get_nome_completo()])) in foto

    normali = 0
    for s1, s2, _punteggio in assegnatore.coppie_formate:
        if _in_foto(s1, s2):
            normali += 1
    ...
    vicino_fisso_nome = None
    nome_vic = getattr(assegnatore, 'nome_adiacente_fisso', None)
    if (getattr(assegnatore, 'studente_fisso', None) and nome_vic
            and nome_vic in vicini_fisso_precedenti):
        vicino_fisso = 1
        vicino_fisso_nome = nome_vic
    ...
    coppia_fisso_nomi = None
    gruppo_adj = getattr(assegnatore, 'gruppo_adiacente_fisso', None)
    if gruppo_adj and len(gruppo_adj) >= 2 and _in_foto(gruppo_adj[0],
                                                        gruppo_adj[1]):
        coppia_fisso = 1
        coppia_fisso_nomi = (gruppo_adj[0].get_nome_completo(),
                             gruppo_adj[1].get_nome_completo())
...
def get_base_path() -> str:
    """Restituisce la cartella base dell'applicazione.

    In un eseguibile PyInstaller usa la directory di ``sys.executable``; in modalità
    script risale dalla cartella ``moduli`` alla radice del progetto.
    """
...
def carica_icona(nome: str) -> QIcon:
    ...
    Le varianti si trovano in ``dati/icone/lucide/<tema>``. Se il tema è sconosciuto usa la variante scura; se il file non è disponibile,
...
def prepara_area_dettagli_popup(
    popup: QMessageBox,
    *,
    larghezza_minima: int = 760,
    altezza_minima: int = 320,
) -> None:
    """Rende leggibile e ridimensionabile l'area dettagli di un QMessageBox.

    Il popup resta compatto finché i dettagli sono chiusi; quando vengono aperti,
    l'area interna riceve dimensioni minime adattate allo schermo.
    """
    popup.setSizeGripEnabled(True)
    schermo_iniziale = popup.screen()
    if schermo_iniziale is not None:
        disponibile_iniziale = schermo_iniziale.availableGeometry()
        larghezza_effettiva = min(
            int(larghezza_minima),
            max(520, int(disponibile_iniziale.width() * 0.70)),
        )
        altezza_effettiva = min(
            int(altezza_minima),
            max(220, int(disponibile_iniziale.height() * 0.42)),
        )
    else:
        larghezza_effettiva = int(larghezza_minima)
        altezza_effettiva = int(altezza_minima)

    popup.setProperty(
        "postiperfettiDettagliLarghezza", larghezza_effettiva
    )
    popup.setProperty(
        "postiperfettiDettagliAltezza", altezza_effettiva
    )

    # Lo stile raggiunge anche il QTextEdit creato internamente da Qt.
    popup.setStyleSheet(
        popup.styleSheet()
        + f"""
        QMessageBox QTextEdit {{
            min-width: {larghezza_effettiva}px;
            min-height: {altezza_effettiva}px;
        }}
        """
    )

    def applica_dimensioni() -> None:
        try:
            layout = popup.layout()
            ...
            aree = popup.findChildren(QTextEdit)
            if not aree:
                return

            area = aree[0]
            schermo = popup.screen()
            larghezza_area = larghezza_effettiva
            altezza_area = altezza_effettiva
            if schermo is not None:
                disponibile = schermo.availableGeometry()

            area.setMinimumSize(larghezza_area, altezza_area)
            area.setSizePolicy(
                QSizePolicy.Policy.Expanding,
                QSizePolicy.Policy.Expanding,
            )

            # Il ridimensionamento scatta soltanto quando i dettagli sono aperti.
            if area.isVisible():
                if schermo is not None:
                    larghezza = min(
                        larghezza_area + 120,
                        int(disponibile.width() * 0.88),
                    )
                    altezza = min(
                        altezza_area + 220,
                        int(disponibile.height() * 0.82),
                    )
                else:
                    larghezza = larghezza_area + 120
                    altezza = altezza_area + 220
                popup.resize(
                    max(popup.width(), larghezza),
                    max(popup.height(), altezza),
                )
        except RuntimeError:
            pass

    def collega_pulsanti() -> None:
        try:
            for pulsante in popup.findChildren(QPushButton):
                if pulsante.property("postiperfettiAggiornaDettagli"):
                    continue
                pulsante.setProperty("postiperfettiAggiornaDettagli", True)
                pulsante.clicked.connect(
                    lambda _checked=False: QTimer.singleShot(
                        0, applica_dimensioni
                    )
                )
            applica_dimensioni()
        except RuntimeError:
    ...
    QTimer.singleShot(0, collega_pulsanti)
...
def pulisci_nome_file(nome: str) -> str:
    ...
    Sostituisce i caratteri vietati e normalizza spazi, trattini e underscore.
    """
    caratteri_vietati = ['/', '\\', ':', '*', '?', '"', '<', '>', '|']
...
def abbrevia_nome_assegnazione(nome_completo: str, data_assegnazione: str = "") -> str:
    ...
    if data_assegnazione:
        try:
            ...
            data_breve = data_assegnazione[-5:]
