...
from moduli.metrica_pulizia import (
    adiacenze_in_fila, conta_riutilizzate_con_foto,
    TIPO_COPPIA, TIPO_TERZETTO, TIPO_QUARTETTO,
)
...
class EsportazioneMixin:
    ...
    def _estrai_nome_completo_da_id(self, id_univoco: str) -> str:
        """Converte l'identificatore di cella nel nome completo leggibile."""
        return nome_completo_da_identificatore(id_univoco)
    ...
    def _verifica_prima_fila_note(self, s1, s2, configurazione_aula):
        ...
        for s in (s1, s2):
            if s.nota_posizione != 'PRIMA':
                continue
            nome_cercato = s.get_nome_completo()
            riga_attuale = None
            for r, riga in enumerate(configurazione_aula.griglia):
                for p in riga:
                    if (p.tipo == 'banco' and p.occupato_da
                            and nome_completo_da_identificatore(p.occupato_da)
                            == nome_cercato):
                        riga_attuale = r
                        break
    ...
    def costruisci_testo_report_terzetti(self, gruppi, motore,
        ...
        studenti = studenti if studenti is not None else self.sessione.studenti
        configurazione_aula = (
            configurazione_aula
            if configurazione_aula is not None
            else self.sessione.aula
        )
        if configurazione_aula is None:
            raise ValueError(
                "Impossibile costruire il report senza una configurazione d'aula."
            )
        data_creazione = (
            data_creazione
        ...
        report.append("-" * 30)
        report.extend(self._righe_layout_aula_testuale(configurazione_aula))
    ...
    def costruisci_testo_report(self, assegnatore, nome_classe=None, studenti=None,
        ...
        testo, la riga identificativa provvisoria, il totale dei riutilizzi e il
        conteggio informativo dei precedenti nell'altro modo.

        Il metodo può essere eseguito dal worker dell'Annuale a coppie: perciò
        non scrive mai attributi della finestra. I chiamanti worker devono
        fornire esplicitamente ``nome_classe`` e ``studenti``.
        """
        if not consenti_fallback_gui and (nome_classe is None or studenti is None):
            raise ValueError(
                "Il report richiesto da un worker deve ricevere nome_classe "
                "e studenti espliciti."
            )
        nome_classe = (
            nome_classe
            if nome_classe is not None
            else self.input_nome_classe.text()
        )
        studenti = (
            studenti
            if studenti is not None
            else self.sessione.studenti
        )
        data_creazione = (
            data_creazione
            if data_creazione is not None
        ...
        numero_coppie = len(assegnatore.coppie_formate)
        report.append(
            "👥 COPPIA FORMATA" if numero_coppie == 1
            else "👥 COPPIE FORMATE"
        )
        report.append("-" * 30)
        if numero_coppie == 0:
            report.append("Nessuna coppia ordinaria: gli studenti sono tutti "
                          "inseriti negli altri blocchi.")
            report.append("")
        ...
        # Il riepilogo conta le note realmente emesse nel dettaglio. Il dato
        # resta locale perché questo builder può girare nel worker annuale.
        precedenti_altro_modo = inserisci_riepilogo_precedenti_altro_modo(
    ...
    def _aggiorna_report_testuale(
        ...
        (testo, riga_identificativa, _riutilizzate_totali,
         precedenti_altro_modo) = self.costruisci_testo_report(
            assegnatore,
            nome_assegnazione=nome_assegnazione,
            data_creazione=data_creazione,
        )

        # Questa assegnazione avviene sul thread GUI.
        self._precedenti_altro_modo = precedenti_altro_modo
        self._riga_identificativa_report = riga_identificativa
    ...
    def esporta_excel(self):
        ...
        """
        if self.sessione.mensile.e_terzetti:
            if not self.sessione.mensile.dati_terzetti:
                self._mostra_errore("Nessun risultato", "Esegui prima un'assegnazione.")
                return
        elif not self.sessione.mensile.assegnatore:
            self._mostra_errore("Nessun risultato", "Esegui prima un'assegnazione.")
            return

        try:
            nome_base = self.sessione.mensile.nome_per_export()
        except RuntimeError as errore:
            self._mostra_errore("Esportazione non disponibile", str(errore))
            return
            ...
            try:
                if self.sessione.mensile.e_terzetti:
                    ...
                    sorgente.configurazione_aula = \
                        self.sessione.mensile.dati_terzetti['configurazione_aula']
                else:
                    sorgente = self.sessione.mensile.assegnatore
                ...
                self._mostra_errore("Errore di esportazione", f"Errore durante l'esportazione:\n{str(e)}")
    ...
    def esporta_report_txt(self):
        ...
        try:
            nome_assegnazione = self.sessione.mensile.nome_per_export()
    ...
    def crea_file_excel(self, file_path: str, assegnatore,
        ...
        if nome_assegnazione is None:
            nome_assegnazione = (
                self.sessione.mensile.nome or self.input_nome_classe.text()
            )
