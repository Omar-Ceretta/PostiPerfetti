...
class Student:
    ...
    def __init__(self, cognome, nome, sesso, nota_posizione="NORMALE"):
        ...
        self.nota_posizione = nota_posizione.strip().upper()  # PRIMA, ULTIMA, NORMALE, FISSO

        # Dizionari per gestire vincoli sociali
        # Chiave: "Cognome Nome" (nome completo dello studente), Valore: livello (1-3)
        # NOTA: si usa il nome completo per evitare ambiguità con cognomi omonimi
        # (es: due "Rossi" nella stessa classe vengono distinti correttamente)
        self.incompatibilita = {}  # Chi NON deve stare vicino
        self.affinita = {}         # Chi DOVREBBE stare vicino

    def aggiungi_incompatibilita(self, nome_completo_studente, livello):
        """
        Aggiunge un'incompatibilità con un altro studente.
        Args:
            nome_completo_studente (str): "Cognome Nome" dello studente incompatibile
            livello (int): Livello di incompatibilità (1-3, dove 3 è assoluto)
        """
        self.incompatibilita[nome_completo_studente.strip()] = int(livello)

    def aggiungi_affinita(self, nome_completo_studente, livello):
        """
        Aggiunge un'affinità con un altro studente.
        Args:
            nome_completo_studente (str): "Cognome Nome" dello studente affine
            livello (int): Livello di affinità (1-3, dove 3 è massimo)
        """
...
def _risolvi_riferimento_completo(riferimento: str, tutti_studenti: list) -> Student:
    ...
    Gestisce cognomi composti e nomi composti.
    NOTA: usata solo per test/debug, non dall'applicazione GUI.
            ...
            if incomp_str.strip():
                ...
                for coppia in incomp_str.split(','):
                    if ':' in coppia:
                        ...
                        if studente_target:
                            # Usa il nome completo "Cognome Nome" come chiave
                            # per evitare ambiguità con cognomi omonimi
                            studente.aggiungi_incompatibilita(studente_target.get_nome_completo(), int(livello))
                ...
                for coppia in aff_str.split(','):
                    if ':' in coppia:
                        ...
                        if studente_target:
                            # Usa il nome completo "Cognome Nome" come chiave
                            # per evitare ambiguità con cognomi omonimi
                            studente.aggiungi_affinita(studente_target.get_nome_completo(), int(livello))
